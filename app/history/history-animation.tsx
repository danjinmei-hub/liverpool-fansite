...
const starts = milestones.map((_, i) => milestones.slice(0, i).reduce((sum, m) => sum + m.duration, 0));
const TOTAL = milestones.reduce((sum, m) => sum + m.duration, 0);
...
function drawFrame(c: Ctx, t: number) {
...
export function HistoryAnimation() {
  ...
  const seek = (index: number) => {
    timeRef.current = starts[index] + 0.001;
    setPlayback(true);
  };
